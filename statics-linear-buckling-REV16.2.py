...
# Copyright (C) 2025  [Il tuo nome o organizzazione]
#
...
class CustomDXFParser:
    ...
    def _extract_loads(self):
        ...
        if len(arrows) >= 3:
            beam = next((name for name, m in self.members.items() 
                        if abs(m['start'][1]-m['end'][1]) < self.TOL['beam_horiz'] 
                        and abs(m['start'][0]-m['end'][0]) > self.TOL['beam_min_len']), None)
            if beam:
                value = -20000
                for text in distributed_texts:
                    nums = re.findall(r'[-+]?\d*\.?\d+', text.text)
                    if nums: value = -float(nums[0])*1000; break
                self.distributed_loads.append({'member':beam, 'value':value, 'direction':'FY', 'class':'Q'})
                print(f"   ✅ Distributed: {beam} q={value:.0f} N/m")
        ...
        for load in self.parser.distributed_loads:
            ...
            if m:
                ...
                if original not in shown_loads:
                    for oname, om in self.parser.members.items():
                        if not om.get('internal') and oname == original:
                            ox1, oy1 = om['start'][:2]; ox2, oy2 = om['end'][:2]
                            ax.text((ox1+ox2)/2, (oy1+oy2)/2+node_size*11, f'q = {abs(load["value"])/1000:.0f} kN/m ({load.get("class","Q")})', fontsize=13, color='darkred', weight='bold', ha='center',
                                   bbox=dict(boxstyle='round,pad=0.5', facecolor='white', edgecolor='red', linewidth=2, alpha=0.95), zorder=15)
                            shown_loads.add(original); break
...
# ============================================================
# FIG 3: DEFORMED SHAPE VIEWER - FINALE CORRETTA
# ============================================================
class DeformedShapeViewer:
    ...
    def _check_equilibrium_and_congruence(self, node_displacements, node_rotations):
        ...
        for node_name in self.parser.supports:
            ...
            if node_name in nodes_m:
                ...
                sum_Mz += Mz + x_m * Ry - y_m * Rx   # <-- CORRETTO
    ...
    def show(self, save_image=True, image_format='png'):
        from scipy.interpolate import PchipInterpolator
        ...
        ax.set_title(f'DEFORMED SHAPE - Max deflection: {max_defl:.2f} mm (×{scale:.0f})', fontsize=16, fontweight='bold')
...
# ============================================================
# FIG 4: STRUCTURAL VERIFICATION + EXCEL (CON CHIAVI MANCANTI AGGIUNTE)
# ============================================================
...
class ResultsExporter:
    ...
    def export(self, filename=None, results=None):
        ...
        auto_width(ws1, 3)
        if results:
            ws5 = wb.create_sheet("SLU - Resistance")
            ws5.cell(row=1, column=1, value="SLU - SECTION RESISTANCE").font = boldL
            ...
            ws6 = wb.create_sheet("SLE - Deflection")
            ws6.cell(row=1, column=1, value="SLE - DEFLECTION").font = boldL
            ...
            for r in results['sle']:
                ...
                row += 1
            auto_width(ws6, 7)
            ws7 = wb.create_sheet("Buckling - Euler")
            ws7.cell(row=1, column=1, value="BUCKLING - EULER").font = boldL
            ...
            for r in results['buckling']:
                ...
                row += 1
            auto_width(ws7, 11)
            ws8 = wb.create_sheet("Beam-Column")
            ws8.cell(row=1, column=1, value="BEAM-COLUMN INTERACTION - Method 2").font = boldL
            ...
            ws9 = wb.create_sheet("Final Summary")
            ws9.cell(row=1, column=1, value="FINAL SUMMARY").font = boldL
            ...
            for r in results['all']:
                slu_info = next((x for x in results['slu'] if x['member']==r['member']), {})
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("="*60)
        print("STRUCTURAL DXF PARSER v16.2 COMPLETE")
    ...
    print("\n⚡ ANALYZING..."); frame.analyze(); print("✅ Done!")
    ...
    print("🔵 FIG 3: Deformed"); DeformedShapeViewer(parser, frame).show()
    ...
    print("\n📊 Exporting Excel...")
    ResultsExporter(parser, frame, gamma_G, gamma_Q, psi_0).export(results=results)
